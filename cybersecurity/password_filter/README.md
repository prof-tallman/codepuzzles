...
Note: This program is a fun challenge but does not adequatly protect the user's input. Do not type in a password for any bank account or important resource.
...
1. ***AI Does My HW***:  
   ...
   - Write a function that will return whether a potential password is at least 8 characters long and contains at least one uppercase letter, one lowercase letter, and one number.
   - Call this function with a few hardcoded sample passwords that meet the requirements and others that fail.

2. ***Script Kiddie***:  
   Hide the password on the screen so that eavesdroppers cannot read it.
   - Replace the hardcoded passwords with a user input prompt that masks the letters typed by the user (see the `getpass` module).
   - In addition to the three previous character classes, require the password to include a symbol like !@#$%^&*().
...
4. ***1337 H@cker***:  
   Improve account security by requiring stricter passwords.
   - Critical resources are usually protected with highly complex passphrases.
...
'bubbles' is not a valid password
...
user@computer:~$ python pwdfilter.py
Enter password:
Success: the supplied password is valid.

user@computer:~$ python pwdfilter.py
Enter password:
Error: the supplied password was too short

user@computer:~$ python pwdfilter.py
Enter password:
Error: the supplied password matched a recent password
```

Here are some additional password examples to help students understand the requirements for the ***1337 Hacker*** stage.
