...
## AI Restrictions ##
Students are allowed to use AI LLMs such as ChatGPT to lookup basic features and examples within the programming language. For example, somebody might ask AI how to convert a letter to a number (in Python it would be the built-in `ord` function; or, a lookup string like `alphabet = 'abcdefghijklmnopqrstuvwxyz'` and the `find` function). Another student might ask how to convert a string entirely to uppercase (e.g., `text = text.upper()` in Python or `text = text.ToUpper()` in C#). Prompts like these are reasonable and allowed.
