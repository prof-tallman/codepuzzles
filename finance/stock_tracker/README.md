# Stock Trader #
Analyze a three-week dataset of stock price bars from a single ticker (e.g., 15-minute closes) in a CSV file. Parse and process the data to calculate summary statistics, generate a report, and create a plot.
...
2. ***Script Kiddie***:  
   ...
   - Use the header row to identify the column index for `Price`.
   - Read every row and convert the `Price` values from strings to numbers.
   - Manually compute (avoid any pre-existing statistics modules):
     - Minimum value
     - Maximum value
...
3. ***Professional***:  
   ...
   - Estimate the price change rate from the last two rows:
     - Report as dollars per hour (use the actual time delta between those two rows to scale to "per hour").
     - Always print a positive rate, and state whether price is rising or falling.
   - Use a position-based command line parameter to specify the CSV filename. Assume future files follow the same structure.
